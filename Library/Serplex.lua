...
, 256
			...
			local function UpdateSliceScale(J)
				local L = not table.find({
					"Shadow-sm",
					"Glass-Circle",
					"Glass-Square"
				}, u) and (J / (256)) or (J / 512)
		...
		function p.Image(v, x, B, C, F, G, H, J)
			...
			elseif string.find(v, "http") then
                ...
                local success, err = pcall(function()
                    if isfile(M) then
                        local ok, asset = pcall(getcustomasset, M)
                        if ok then
                            L.ImageLabel.Image = asset
                            return
                        end
                    end

                    local body = p.Request({
                        Url = v,
                        Method = "GET",
                    }).Body
                    writefile(M, body)
                ...
                if not success then
                    warn("[ WindUI.Creator ] URL Image failed:", err)
                    L.Visible = false
                end                
			elseif v == "" then
				L.Visible = false
		...
		function ae.Create(af, ag)
			...
			function ah.Open(ai)
				...
				task.spawn(function()
					...
					if not af then
						ac(ah.UIElements.FullScreen, 0.1, {
							BackgroundTransparency = .3
						}):Play()
					end
					ac(ah.UIElements.MainContainer, 0.1, {
						ImageTransparency = 0
					}):Play()
					task.spawn(function()
						task.wait(0.05)
...
and ag.Color
			...
			if ag.Hover then
				aa.AddSignal(aC.MouseEnter, function()
					if aj then
						ad(aC, .12, {
							ImageTransparency = ag.Color and .15 or .9
						}):Play()
						ad(aA, .12, {
							ImageTransparency = .9
						}):Play()
...
"SliderIconTo"
							...
							an = aa.InputEnded:Connect(function(aD)
								if (aD.UserInputType == Enum.UserInputType.MouseButton1 or aD.UserInputType == Enum.UserInputType.Touch) and aA == aD then
									...
									if aj.Window.NewElements then
										af(ak.UIElements.SliderIcon.Frame.Thumb, .2, {
											ImageTransparency = 0,
											Size = UDim2.new(0, aj.Window.NewElements and (ak.ThumbSize * 2) or (ak.ThumbSize + 2), 0, aj.Window.NewElements and (ak.ThumbSize + 4) or (ak.ThumbSize + 2))
			...
			ad.AddSignal(ak.UIElements.SliderContainer.InputBegan, function(ay)
				...
				if ay.UserInputType == Enum.UserInputType.MouseButton1 or ay.UserInputType == Enum.UserInputType.Touch then
					if aj.Window.NewElements then
						af(ak.UIElements.SliderIcon.Frame.Thumb, .24, {
							ImageTransparency = .85,
...
42 + (au.UIPadding)
				...
				function au.User.Enable(aC)
					...
					am(au.UIElements.SideBarContainer, .25, {
...
if game:GetService('CoreGui'):FindFirstChild('Serplex') then
   game:GetService('CoreGui'):FindFirstChild('Serplex'):Destroy()
end
